...
from .heuristics import shape_heuristic
...
def breaks_symmetry(prefix: List[Op], next_op: Op) -> bool:
    ...
    if hasattr(prefix[-1], 'commutes_with') and next_op.name in prefix[-1].commutes_with:
...
def extend_with_op(prefix_states: List[Grid], target_states: List[Grid], op: Op, remaining: int, op_timeout: float = 0.25) -> Optional[List[Grid]]:
    """
    ...
    for i, (grid, target) in enumerate(zip(prefix_states, target_states)):
        try:
            with time_limit(op_timeout):
                ...
                # ----------   H / W  disagreement prune  ----------
                delta           = np.sum(new_grid.data != target.data)
                # very loose upper-bound: one op may touch *all* cells
                if delta > remaining * new_grid.data.size:
                    return None
...
def enumerate_programs(primitives: List[Op], prefix: List[Op], remaining: int,
    ...
    for op in primitives:
        ...
        if train_inputs and train_outputs and prefix_states:
            ...
            new_states = extend_with_op(prefix_states, train_outputs, op, remaining, op_timeout)
